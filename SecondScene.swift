...
class SecondScene: SKScene{
    ...
    let terrain = SKShapeNode(ellipseOf: CGSize(width: 240, height: 100))
    ...
    override func didMove(to view: SKView) {
        ...
        */
        ...
        player.position = .init(x:frame.width/2, y:frame.height/2)
        addChild(player)
        ...
        bottomBorder.position = .init(x:image.size.width/2, y: 100)
        ...
        topBorder.position = .init(x:image.size.width/2, y: image.size.height-50)
        addChild(topBorder)
        
        leftBorder.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: 2, height: frame.height))
        leftBorder.physicsBody?.affectedByGravity = false
        leftBorder.physicsBody?.isDynamic = false
        leftBorder.position = .init(x:200, y: image.size.height/2)
        addChild(leftBorder)
        ...
        rightBorder.position = .init(x:image.size.width-100, y: image.size.height/2)
